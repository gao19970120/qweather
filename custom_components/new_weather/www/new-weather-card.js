...
if (!customElements.get('new-weather-pad-card')) {
  customElements.define('new-weather-pad-card', XiaoshiWeatherPadCardV2);
}
...
class QWeatherGlanceCard extends LitElement {
  ...
  setConfig(config) {
    if (!config || !config.entity) {
      throw new Error('new-weather-glance-card 需要指定天气实体');
    }
    this.config = {
      sun_entity: 'sun.sun',
  ...
  _syncClockWeatherCard() {
    ...
    if (!clockCard._qweatherConfigApplied || JSON.stringify(clockCard._qweatherConfigApplied) !== JSON.stringify(childConfig)) {
      try {
        ...
        console.error('[new-weather-glance-card] clock-weather-card 配置失败:', err);
      }
    }
...
if (!customElements.get('new-weather-glance-card')) {
  customElements.define('new-weather-glance-card', QWeatherGlanceCard);
}

window.customCards = window.customCards || [];
window.customCards.push(
  {
    type: "new-weather-glance-card",
    name: "和风天气外层概览卡",
    preview: true
  },
  {
